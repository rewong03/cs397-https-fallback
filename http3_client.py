...
#         assert _host == host, f"URL hostname doesn't match ({_host}, {host})"
...
async def send_request(
    ...
    async with connect(
        ...
        if output_dir is not None:
            ...
            file_name = "".join([c for c in url if c.isalnum()]) + "_res"
            # output_path = os.path.join(
            #     output_dir, os.path.basename(urlparse(url).path) or "index.html"
            # )
            file_no = 0
            output_path_raw = os.path.join(output_dir, file_name)
            output_path = output_path_raw + f"{file_no}"
            while os.path.exists(output_path):
                file_no += 1
                output_path = output_path_raw + f"{file_no}"

            with open(output_path, "wb") as output_file:
                output_file.write(str(headers).encode() + b"\n\n")
                if response.status_code != 200:
                    print(f"{url} returned bad code {response.status_code}")
                if include:
                    headers = ""
                    headers += f"Status code: {response.status_code}\r\n"
